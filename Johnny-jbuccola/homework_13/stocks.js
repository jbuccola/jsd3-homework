...
function Stock (symbol, shares) {
  ...
  this.shares = shares;
  this.exchange = 'NYSE';
  this.buyShares = function(qty) {
    this.shares += qty;
  };
  this.sellShares = function(qty) {
    if ((this.shares - qty) < 0) {
      console.log("The sale amount requested exceeds the total remaining shares. No stocks were sold");
    } else {
      this.shares -= qty;
    }
  };
  this.calcValue = function(price) {
    var total = (this.shares * price);
    var rounded = total.toFixed(2);
    console.log("The total value of your stock at " + price + " per share is " + rounded);
  };
}
